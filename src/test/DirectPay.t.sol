...
import { DirectPay, DirectPayRules } from "src/contracts/Modules/DirectPay.sol";
...
contract DirectPayTest is Test {
    ...
    function testWhitelistToken() public {
        ...
        assertFalse(REGISTRAR.ruleWhitelisted(directPayRulesAddress), "Unauthorized whitelist");
        REGISTRAR.whitelistRule(directPayRulesAddress, true); // whitelist bytecode, not address
        assertTrue(REGISTRAR.ruleWhitelisted(directPayRulesAddress), "Whitelisting failed");
        REGISTRAR.whitelistRule(directPayRulesAddress, false);
        assertFalse(REGISTRAR.ruleWhitelisted(directPayRulesAddress), "Un-whitelisting failed");
        REGISTRAR.whitelistRule(directPayRulesAddress, true); // whitelist bytecode, not address

        // Whitelist module
        DirectPay directPay = new DirectPay(address(REGISTRAR), directPayRulesAddress, directPayRulesAddress, directPayRulesAddress, directPayRulesAddress, directPayRulesAddress, DataTypes.WTFCFees(0,0,0,0), "ipfs://yourmemos.com/");
        address directPayAddress = address(directPay);
        (bool addressWhitelisted, bool bytecodeWhitelisted) = REGISTRAR.moduleWhitelisted(directPayAddress);
        ...
        (addressWhitelisted, bytecodeWhitelisted) = REGISTRAR.moduleWhitelisted(directPayAddress);
    ...
    function setUpDirectPay() public returns (DirectPay){  // Deploy and whitelist module
        DirectPayRules directPayRules = new DirectPayRules();
    ...
    function testWritePay(  // escrow==amount && drawer==caller && recipient==owner
        ...
        bytes memory initData = abi.encode(bytes32(keccak256("this is a hash")), caller);

        vm.prank(caller); 
        uint256 cheqId = REGISTRAR.write(cheq, owner, directAmount, initData);  // Sets caller as owner
        registrarWriteAfter(cheqId, amount, 0, owner, drawer, recipient, address(directPay));
    ...
    function testWriteInvoice(  // escrow==0 && drawer==caller && drawer==owner
        ...
        bytes memory initData = abi.encode(bytes32(keccak256("this is another hash")), caller);
    ...
    function calcTotalFees(CheqRegistrar registrar, DirectPay directPay, uint256 escrowed, uint256 directAmount) public returns(uint256) {
        (uint256 writeFeeBPS, , , ) = registrar.getFees();
        (uint256 moduleWriteFeeBPS, , , ) = directPay.getFees();
        ...
        console.log("ModuleFee: ", moduleFee);
        uint256 totalWithFees = escrowed + directAmount + registrarFee + moduleFee;
        console.log(directAmount, "-->", totalWithFees);
        return totalWithFees;
    }

    function writeHelper(address caller, uint256 amount, uint256 escrowed, uint256 directAmount, address drawer, address recipient, address owner) public returns(uint256, DirectPay){ 
        DirectPay directPay = setUpDirectPay();

        uint256 totalWithFees = calcTotalFees(REGISTRAR, directPay, escrowed, directAmount);
        REGISTRAR.whitelistToken(address(dai), true);
        vm.prank(caller); 
        dai.approve(address(REGISTRAR), totalWithFees);  // Need to get the fee amounts beforehand
        dai.transfer(caller, totalWithFees);
    ...
    function testFundInvoice(address caller, uint256 amount, address drawer, address recipient) public {
        vm.assume(amount != 0 && amount <= tokensCreated);
        (address drawer, uint256 escrowed, uint256 directAmount, address owner) = (caller, 0, amount, caller);
        vm.assume(caller != address(0) && recipient != address(0) && !isContract(owner));
        vm.assume(drawer != recipient);

        (uint256 cheqId, DirectPay directPay) = writeHelper(caller, amount, escrowed, directAmount, drawer, recipient, owner);
        bytes memory fundData =  abi.encode(bytes32(""));
