...
contract ContractTest is Test {
    ...
    function setUp() public { 
        ...
        dai = new TestERC20(1_000_000_000_000_000_000_000_000_000_000e18, "DAI", "DAI");  // Sends ContractTest the dai
        usdc = new TestERC20(0, "USDC", "USDC");
    }
    ...
    function setUpBrokers() public{
        SelfSignTimeLock selfSignedTL = new SelfSignTimeLock(cheq);
        cheq.whitelistBroker(selfSignedTL, true);
    }

    function testDeposit(uint256 _amount) public {
        if (_amount<=1_000_000_000_000_000_000_000_000_000_000e18){
        dai.approve(address(cheq), _amount+1);
        assertTrue(dai.allowance(address(this), address(cheq))==_amount+1);

        cheq.deposit(dai, _amount+1);
        assertTrue(cheq.deposits(address(this), dai)==_amount+1);
        }
    }
    ...
    function depositHelper(uint256 _amount, address _to) public {
        dai.approve(address(cheq), _amount+1);
        cheq.deposit(_to, dai, _amount+1);
    }

    function testWrite(uint256 _amount, address _to) public {
        depositHelper(_amount, _to);
        // cheq.write();  // Send a normal cheq
    }

    function testFailWrite() public {
    }
