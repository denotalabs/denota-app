...
contract SelfSignTimeLock is ICheqModule, Ownable {
    ...
    function writeCheq(
        // Integer overflows if block.timestamp+inspectionPeriod too large, solidity 0.8 reverts
        IERC20 _token,
        uint256 amount,
        uint256 escrow,
        address recipient,
        ...
        if (escrow == 0) {
            ...
            if (cheq.deposits(_msgSender(), _token) < escrow) {
                uint256 shortfall = escrow -
                    cheq.deposits(_msgSender(), _token);
                require(
                    cheq.deposit(_msgSender(), _token, shortfall),
                    "Deposit failed"
                );
            }
