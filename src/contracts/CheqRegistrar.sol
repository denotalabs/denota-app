...
contract CheqRegistrar is ERC721, Ownable {
    ...
    modifier onlyModule(uint256 cheqId) {
        require(
            _msgSender() == address(cheqInfo[cheqId].module),
            "Only cheq's module"
        );
        _;
    }
    modifier userWhitelisted(ICheqModule module) {
        require(
            userModuleWhitelist[_msgSender()][module],
            "Only whitelisted modules"
        );
        _;
    } // TODO User whitelist this
    ...
    /*//////////////////////////////////////////////////////////////
                            USER DEPOSITS
    //////////////////////////////////////////////////////////////*/
    function _deposit(IERC20 token, address to, uint256 amount) private {
    ...
    function deposit(IERC20 _token, uint256 _amount) public returns (bool) {
        // make one external and use other in depositWrite()?
    ...
    function write(
        ...
        if (from != _msgSender()) {
            ...
            require(
                userModuleWhitelist[from][ICheqModule(_msgSender())],
                "UNAPP_MODULE"
            ); // See if user allows this
        }
        require(escrow <= deposits[from][_token], "INSUF_BAL");
    ...
    function transferFrom(
        ...
        address to,
        uint256 cheqId
    ) public virtual override onlyModule(cheqId) {
    ...
    function approve(
        address to,
        uint256 tokenId
    ) public virtual override onlyModule(tokenId) {
