...
contract FlanTokenStaking is Context, Ownable, ReentrancyGuard {
    ...
    function stake(uint256 amount_) external payable {
        // FIXME: Payable not needed since native is not being transfered
        require(msg.sender != address(0x0), "");
        require(amount_ > 0, "");
        if (stakingAddressAmount[msg.sender].totalAmount == 0) {
            // FIXME: this can be simplified to the 4th line
    ...
    function withdrawAmount(uint256 amount_) external payable {
        // Question: Not sure why this is payable
    ...
    function _withdrawAmount(address payable address_, uint256 amount_)
        internal
        virtual
        returns (uint256)
    {
        // Question: Why is the address payable if native tokens aren't being used?
        require(amount_ > 0, "Flan Staking: requested amount == 0");
        require(
            stakingAddressAmount[address_].totalAmount >= amount_,
            "Flan Staking: Balance < requested amount"
        );
        require(
            _getCurrentTime() >=
                stakingAddressAmount[address_].actionTime + _minLockDay * 3600,
            "Flan Staking: Lock Period"
        );
    ...
    function getAmountOfMember(address address_)
        public
        view
    ...
    function getAddressMinUnlockTime(address address_)
        public
        view
        returns (uint32)
    {
    ...
    function _getAddressAmount(address address_)
        private
        view
        returns (uint256, uint32)
    {
        return (
            stakingAddressAmount[address_].totalAmount,
            stakingAddressAmount[address_].actionTime
        );
    }

    function _getAddressMinUnlockTime(address address_)
        private
        view
        returns (uint32)
    {
...
contract FlanPayment is Context, Ownable, ReentrancyGuard {
    ...
    struct PaymentData {
        // Invoice data Question 1) should cheqId == projectId and taskId == milestones?
        string projectId;
        string taskId;
    ...
    event Paid(
        uint256 paymentId_,
        address indexed senderAddress_,
        address indexed receiptAddress_,
        string projectId_,
        string taskId_,
        uint256 taskEstimate_,
        uint256 feeAmount_,
        bool isFLN_,
        uint32 paidTime
    );
    ...
    constructor(
        address flanAddress_,
        address cUSDAddress_,
    ...
    function init() private {
        // Not efficient with many tiers, NOTE tiers can't be continueous (Fee = f(x))
        feePlanList.push(
            FeePlan({
                name: "F0 Tier",
    ...
    function setStakingContract(address stakingContractAddress_)
        external
        onlyOwner
    {
        require(stakingContractAddress_ != address(0x0), "");
    ...
    function getStakedAmount(address address_) private view returns (uint256) {
        (
            uint256 amount, /*uint32 time*/

        ) = FlanTokenStaking(_stakingContractAddress).getAmountOfMember(
                address_
            );
        return amount;
    }
    ...
    function pay(
        ...
        for (uint256 i = 0; i < feePlanList.length; i++) {
            // FIXME: This can be done more efficiently. General looping opt. and break statement
            if (feePlanList[i].minStakedAmount <= stakedAmount)
                freelancerGrade = i;
        }
        uint256 taxFeeAmount = (taskEstimate * 5) / 1000; // 0.5% fee <=> .95taskEstimate
        uint256 feeAmount = 0;
        if (isFLN) {
            // FIXME: Code here is duplicated NOTE: better handled by write(){ module.processWrite(require(whitelisted[cheq.currency])) }  // NOTE no way to add additional tokens
            feeAmount =
                (taskEstimate * feePlanList[freelancerGrade].flanFeePercent) /
                100;
            _flanToken.safeTransferFrom(
                msg.sender,
                address(this),
                taskEstimate
            ); // The payer's full token amount is sent to the module
            _flanToken.safeTransfer(
                receiptAddress,
                taskEstimate - feeAmount - taxFeeAmount
            ); // The module then send the recipient the tokens minus the fees
            _totalFLNFeeAmount += feeAmount;
            _FLNTaxFeeAmount += taxFeeAmount;
        } else {
            feeAmount =
                (taskEstimate * feePlanList[freelancerGrade].cUSDFeePercent) /
                100;
            _cusdToken.safeTransferFrom(
                msg.sender,
                address(this),
                taskEstimate
            );
            _cusdToken.safeTransfer(
                receiptAddress,
                taskEstimate - feeAmount - taxFeeAmount
            );
    ...
    function withdrawAllCUSD() external onlyOwner {
        // FIXME: Having two separate functions is a bit redundant
    ...
    function withdrawOnlyTaxFee() external onlyOwner {
        // Question: why segregate tax fees and payment fees? If Flan.design pays taxes they can just take taxes on total off-chain
    ...
    function getPayment(uint256 paymentId_)
        public
        view
        returns (
            string memory,
    ...
    function getPaymentId() public view returns (uint256) {
        // Same as getting totalSupply(). NOTE: Module operators may want the number of payments processed on their platform and may be opinionated as to where to query (blockchain or subgraph)
        return _paymentId;
    }
